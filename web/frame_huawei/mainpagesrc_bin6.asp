...
var varSingtel = '<%HW_WEB_GetFeatureSupport(FT_FEATURE_SINGTEL);%>';
var varSingtelHg8244hs = '<%HW_WEB_GetFeatureSupport(VOICE_FT_WEB_SINGTEL_HG8244HS);%>';
...
function ChangeClickConfigDiv(ShowFlag, PageIdType, PointerId)
{
    ...
    for(var index = 0; index < gCondetailInfo.length - 1; index++) {
        ...
        if (PageIdType == gCondetailInfo[index].clicktType) {
            ...
            if (gCondetailInfo[index].iframeid != undefined
               && gCondetailInfo[index].frameurl != undefined) {
...
var FrameMainPage = {
    ...
    InitMainpageDesInfo:function(){
        ...
        if (IsSupportWifi() == true && IsSupportVoice() == true) {
           ...
           $("#WIFIIconInfo").click(function() {
            ...
            this.InitPhoneDevInfo();
            return;
        } else if (IsSupportWifi() == false && IsSupportVoice() == false) {
    ...
    InitLineDevCntInfoWithoutWIFI:function(){
        ...
        var LineIndex = 0;
        for (var index = 0; index < gCondetailInfo.length - 1; index++) {
            if (gCondetailInfo[index].clicktType == "wifidev") {
                wifiIndex = index;
            }

            if (gCondetailInfo[index].clicktType == "linedev") {
                LineIndex = index;
            }
        }
    ...
    InitLineDevInfowithoutvoip:function(){
        ...
        if ((varSingtel == true) && (varSingtelHg8244hs == false)) {
        ...
        var LineIndex = 0;
        for(var index = 0; index < gCondetailInfo.length - 1; index++) {
            if (gCondetailInfo[index].clicktType == "teldev") {
                TelIndex = index;
            }
    ...
    InitUSBInfo:function(){
        var frame = this;
        if (IsSupportSamba()) {    
            if (IGMPEnable == 0 && parseInt(varSingtel) == 1) {
