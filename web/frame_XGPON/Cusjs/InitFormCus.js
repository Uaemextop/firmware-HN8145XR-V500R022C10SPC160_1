...
function HWAppendTitle(TitleId, Titlestring, SummaryInfo, IsneedImg)
{
	...
	if (null != Titlestring)
	{
		titlehtml ='<div id="' + TitleId + 'title" class="PageTitle_title">' + Titlestring + '</div>';
	}
	...
	else
	{
		...
		for (var index = 0; index < SummaryLen; index++)
		{
			...
			else
			{
				...
				{
					var icontextstring = '<div class="divpageicontext"><span class="pageicontext">'
										+ SummaryInfo[index].icontext
										+ '</span></div>';
				}
...
function CreateTableStartString(tableinfo)
{
	...
	var StartTable ='<table id="' + tableinfo + '" width="100%" cellpadding="0" cellspacing="0" class="tabal_noborder_bg">';
...
function gotoIndex()
{
  ...
  window.localStorage.setItem('guide', 'Wan');
