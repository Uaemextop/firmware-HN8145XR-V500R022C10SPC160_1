...
HW_Script_SetDatToFile()
{
	...
	if [ $var_support_secure_feature = 1 ]; then
		HW_Script_Cfgtool_Set $var_default_ctree $var_node_admin_web_pwd "Password" $var_web_pwd

		#设置管理员用户出厂密码，没有就新增
		cfgtool gettofile $var_default_ctree $var_node_admin_web_pwd "FactoryPassword"
		if [ 0 -eq $? ]
		then
			cfgtool set $var_default_ctree $var_node_admin_web_pwd "FactoryPassword" $var_web_pwd
		else
			cfgtool add $var_default_ctree $var_node_admin_web_pwd "FactoryPassword" $var_web_pwd
		fi
