...
var mainMenuJsonObject = {
	...
	"nowifi" : {"mainDivId":"nowifi_main_div", "hasChildMenu":false, "src": "/cgi-bin/luci/admin/wifi/mesh", "title":"组网设置", "selectClass": "first_menu_primary_select"},
	...
	"appconfig" : {"mainDivId":"appconfig_main_div", "hasChildMenu":true, "title":"更多应用", "selectClass": "first_menu_more_select"}
};
var subMenuJsonObject = {
	...
	"wifi": {
		...
		"secondmenus":[
			...
			{"name": "inter_connd", "title":"扩展WiFi", "src": "/cgi-bin/luci/admin/wifi/inter_connd"},
			{"name": "easymesh", "title":"MESH配置", "src": "/cgi-bin/luci/admin/wifi/mesh"},
		]
	},	
	"storage": {
		"childMenuPosition": "top",
	...
	"setting": {
		...
		"secondmenus":[
			...
			{"name": "ddns", "title":"DDNS设置", "src": "/cgi-bin/luci/admin/settings/ddns"},
			{"name": "iptv", "title":"IPTV显示", "src": "/cgi-bin/luci/admin/settings/iptv"},
			...
			{"name": "restore", "title":"网关重置", "src": "/cgi-bin/luci/admin/settings/restore"},
			{"name": "restore_framework", "title":"中间件重置", "src": "/cgi-bin/luci/admin/settings/fwrestore"}
		]
	},
	"appconfig": {
		"childMenuPosition": "left",
		"secondmenus":[
			{"name": "app_hot", "title":"应用商城", "src": "/cgi-bin/luci/admin/appconfig/mall"}
		]
	}
};
...
function constructSubMenuHTML(name)
{
	...
	if (subMenuObj != undefined)
	{
		...
		for (var i=0; i<subMenuObj.secondmenus.length; i++) //二级菜单
		{
			...
			if (noWiFi && (subMenuObj.secondmenus[i].name == "wifi"))
				continue;

			if ((noPots || (devType == 13)) && (subMenuObj.secondmenus[i].name == "phone"))
				continue;

			if ((devType == 13) && ((subMenuObj.secondmenus[i].name == "advance") || (subMenuObj.secondmenus[i].name == "inter_connd") || (subMenuObj.secondmenus[i].name == "easymesh")))
				continue;			

			if (!noWiFi && noMesh && (subMenuObj.secondmenus[i].name == "easymesh"))
				continue;

			if ((devType != 13) && (subMenuObj.secondmenus[i].name == "iptv"))
				continue;
