...
$(document).ready(function(){
	...
	$("#btn_login_step_1").click(function(){
			...
			if (isWiFi)
			{
				$("#login_WIFI_SSID").val(ssid);
				$("#btn_login_step_2").removeAttr("disabled");
				$(".conf-content").children(":first").find("span").text("天翼网关"+productCls);
			...
			else
			{
				...
				$("#login_setup_1_div").css("display", "none");
				(new XHR()).post('/cgi-bin/luci/admin/settings/setLoid',{ loidvalue: $("#loid").val(), loidpassword : $("#loidPwd").val(), wifissid : "", wifipassword : "", hasWiFi : 0},
				function(x)
				{
	...
	$("#btn_login_step_2").click(function(){
			...
			if ((ssid != wifiSsid) || (wifiPwd.length > 0))
			{
				...
				else if (((beacon == "WPA") || (beacon == "WPA2") || (beacon == "WPA/WPA2")) && ((wifiPwd.length < 8) || (wifiPwd.length > 32) || !isValidWPAKey(wifiPwd)))
				{
					if ( $("#login_WIFI_SSID").parent().parent().children().length > 3)
					  	$("#login_WIFI_SSID").parent().parent().children(":last").remove();			
					$("#login_WIFI_SSID").parent().parent().children(":last").after("<div><font face=\"宋体\" style=\"font-size: 14px; color:#aaa\">无线当前认证方式要求密码应该在8到32个字符之间且只能为数字或者a~zA~Z之间字符</font></div>");				
					return false;				
				}
		    }
			
			$("#login_setup_2_div").css("display", "none");
			(new XHR()).post('/cgi-bin/luci/admin/settings/setLoid',{ loidvalue: $("#loid").val(), loidpassword : $("#loidPwd").val(), wifissid : $("#login_WIFI_SSID").val(), wifipassword : wifiPwd, hasWiFi : 1},
			function(x)
			{
					var retinfo = eval("("+x.responseText+")");
					if(retinfo.retVal == 0)
