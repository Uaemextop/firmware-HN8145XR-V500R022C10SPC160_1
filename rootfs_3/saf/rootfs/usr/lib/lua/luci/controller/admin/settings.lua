...
function index()
	...
	entry({"admin", "settings", "ddns"},
		template("admin_settings/main_ddns_config"),
		nil, 50).leaf = true	

	entry({"admin", "settings", "ddnsInfo"}, call("get_ddns_settings")).leaf = true	
	
	entry({"admin", "settings", "ddnsSettings"}, post("set_ddns_settings")).leaf = true

	entry({"admin", "settings", "iptv"},
		template("admin_settings/main_iptv_config"),
		nil, 55).leaf = true

	entry({"admin", "settings", "iptvInfo"}, call("get_iptv_settings")).leaf = true	
	
	entry({"admin", "settings", "iptvSettings"}, post("set_iptv_settings")).leaf = true		
	...
	entry({"admin", "settings", "fwrestore"},
		template("admin_settings/main_restore_framework"),
		nil, 150).leaf = true		
	...
	entry({"admin", "settings", "doFwRestore"}, post("do_fwrestore")).leaf = true
end
...
function set_lan_settings()
	...
	if ret ~= 2 then
		...
		os.execute("killall -9 uhttpd >/dev/null 2>&1")
	end
end

function get_ddns_settings()
	local info = sgwconfig:get("ddns")

	luci.http.prepare_content("application/json")
	luci.http.write_json(info)
end

function set_ddns_settings()
	local tbl
	tbl = {provider = luci.http.formvalue("provider"),
	       enable = luci.http.formvalue("enable"),
	       domain = luci.http.formvalue("domain"), 
	       hostname = luci.http.formvalue("hostname"), 
	       username = luci.http.formvalue("username"),
	       passwd = luci.http.formvalue("passwd"),
	       add = luci.http.formvalue("isAdd")}
	       
	sgwconfig:set("ddns", tbl)
end

function get_iptv_settings()
	local info = sgwconfig:get("iptv")

	luci.http.prepare_content("application/json")
	luci.http.write_json(info)
end

function set_iptv_settings()
	local tbl
	tbl = {enable = luci.http.formvalue("enable"),
	       vlanOnOff = luci.http.formvalue("vlanOnOff"), 
	       vlanid = luci.http.formvalue("vlanid"), 
	       bindlanport = luci.http.formvalue("bindlanport"),
	       bindwlan2port = luci.http.formvalue("bindwlan2port"),
	       bindwlan5port = luci.http.formvalue("bindwlan5port")}
	       
	sgwconfig:set("iptv", tbl)
end
...
function do_fwrestore()
  local ret = sgwconfig:set("fwrestore")
...
function set_loid()	
	...
	tbl = {
			...
			hasWiFi = luci.http.formvalue("hasWiFi")
		   }
