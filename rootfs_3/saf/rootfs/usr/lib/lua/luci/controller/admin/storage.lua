...
function index()
	...
	entry({"admin", "storage", "getFiles"}, post("get_file_lists")).leaf = true
	
	entry({"admin", "storage", "getSelectedUsb"}, post("get_selected_usb")).leaf = true

	entry({"admin", "storage", "openFolder"}, post("open_folder")).leaf = true

	entry({"admin", "storage", "newFolder"}, post("new_folder")).leaf = true

	entry({"admin", "storage", "deleteFiles"}, post("delete_files")).leaf = true

	entry({"admin", "storage", "rename"}, post("rename_file")).leaf = true

	entry({"admin", "storage", "getDiagFolders"}, post("get_diag_folders")).leaf = true  
	
 	entry({"admin", "storage", "copyMove"}, post("copy_move_files")).leaf = true

 	entry({"admin", "storage", "getProgress"}, post("copy_move_progress")).leaf = true

 	entry({"admin", "storage", "usbFormat"}, post("usb_dev_format")).leaf = true
end
