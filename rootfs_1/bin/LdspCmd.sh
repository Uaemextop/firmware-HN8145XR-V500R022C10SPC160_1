...
help_ploam()
{
		...
		echo "                 log on  --- Open log switch of ploam register message"
		echo "                 log off --- Close log switch of ploam register message"
...
help_mpcp()
{
		...
		echo "                 log on/off --- Open/Close the log switch of MPCP"
...
{              
		echo "		mbist"
		echo "			-h  --- Get help of mbist Module"
		echo "			test [times]   --- set mbist test times, reset for start test"
		echo "			display result --- Display mbist test result"
		echo ""
}
...
{
		echo "         pcie"
		echo "                 -h  --- Get help of pcie test"
		echo "                 maskwifi -- mask wifi for pcie"
	        echo "                 loopback <enable/disable> -- enable/disable the pcie loopback test"
		echo "                 test [id] [memsize] [times]  -- pcie loopback test start"
		echo "                 result -- get the pcie loopback test result"
		echo ""
}
...
{
		echo "         serdes"
		echo "                 -h  --- Get help of serdes test"
	        echo "                 dfxtest [times] -- serdes eye diagram scan times"
		echo "                 result -- get the eye diagram test result"
		echo "                 loopback <enable | disable> --  serdes loopback enable or disable"		
		echo ""
}
	...
	if [ "$1" = "-h" ];then
		...
		if [ -e /var/epon_up_mode.txt -o -e /var/ge_up_mode.txt ];then
			...
			check_param 3 || exit $?
			if [ "$3" = "on" ];then
				send_hlp 0 0x2000300b 0xb 12 0 1 0 || exit $?
			elif [ "$3" = "off" ];then
				send_hlp 0 0x2000300b 0xb 12 0 0 0 || exit $?
			else
				echo "ERROR::input para is not right!"
				exit 1
			fi 
			...
			check_param 3 || exit $?
			if [ "$3" = "on" ];then
				send_hlp 0 0x2000300b 0xb 12 1 1 0 || exit $?
			elif [ "$3" = "off" ];then
				send_hlp 0 0x2000300b 0xb 12 1 0 0 || exit $?
			else
				echo "ERROR::input para is not right!"
				exit 1			
			fi 
			...
			check_param 3 || exit $?
			if [ "$3" = "on" ];then
				send_hlp 0 0x2000300b 0xb 12 2 1 0 || exit $?
			elif [ "$3" = "off" ];then
				send_hlp 0 0x2000300b 0xb 12 2 0 0 || exit $?
			else
				echo "ERROR::input para is not right!"
				exit 1			
			fi 
			...
			check_param 3 || exit $?
			if [ "$3" = "on" ];then
				send_hlp 0 0x2000300b 0xb 12 3 1 0 || exit $?
			elif [ "$3" = "off" ];then
				send_hlp 0 0x2000300b 0xb 12 3 0 0 || exit $?
			else
				echo "ERROR::input para is not right!"
				exit 1				
			fi 
			...
			check_param 3 || exit $?
			if [ "$3" = "on" ];then
				send_hlp 0 0x2000300b 0xb 12 4 1 0 || exit $?
			elif [ "$3" = "off" ];then
				send_hlp 0 0x2000300b 0xb 12 4 0 0 || exit $?
			else
				echo "ERROR::input para is not right!"
				exit 1				
			fi 
			...
			check_param 3 || exit $?
			if [ "$3" = "stat" ];then
				send_hlp 0 0x2000300d 0xd 4 0 || exit $?
			else
				echo "ERROR::input para is not right!"
				exit 1					
			fi
			...
			check_param 3 || exit $?
			if [ "$3" = "log" ];then
				send_hlp 0 0x20003016 0x16 4 0 || exit $?
			else
				echo "ERROR::input para is not right!"
				exit 1					
			fi
		...
		if [ "$2" = "-h" ];then
			...
			if [ "$3" = "A0" ];then
				send_hlp 100 0x20013000 0 18 0x0 0x"$4"0050 0x20000 0x2 0x0 || exit $?
			elif [ "$3" = "A2" ];then
				send_hlp 100 0x20013000 0 18 0x0 0x"$4"0051 0x20000 0x2 0x0 || exit $?
			elif [ "$3" = "A4" ];then
				send_hlp 100 0x20013000 0 18 0x0 0x"$4"0052 0x20000 0x2 0x0 || exit $?
			elif [ "$3" = "A6" ];then
				send_hlp 100 0x20013000 0 18 0x0 0x"$4"0053 0x20000 0x2 0x0 || exit $?
			else
				echo "ERROR::input para is not right!"
				exit 1					
			fi
		...
		if [ -e /var/gpon_up_mode.txt -o -e /var/ge_up_mode.txt ];then
			...
			check_param 3 || exit $?
			if [ "$3" = "on" ];then
				send_hlp 1000 0x20031006 0x6 8 3 1 || exit $?
				
			elif [ "$3" = "off" ];then
				send_hlp 1000 0x20031006 0x6 8 3 0 || exit $?
		...
		if [ "$2" = "print" ];then
			...
			if [ "$3" = "hlp" ];then
				...
				send_hlp 0 0x20000000 0 12 27 "$4" "$5" || exit $?
			elif [ "$3" = "acl" ];then
                exit 1
			elif [ "$3" = "napt" ];then
                exit 1
		...
		if [ "$2" = "-h" ];then
			...
			help_pcie
			exit 0
		elif [ "$2" = "loopback" ];then
			check_param 3 || exit $?
			if [ "$3" = "enable" ];then
				send_hlp_check  100 0x20004037 0x37 16 0 1 0 0|| exit $?
				send_hlp_check  100 0x20004037 0x37 16 1 1 0 0|| exit $?
			elif [ "$3" = "disable" ];then
				send_hlp_check  100 0x20004037 0x37 16 0 0 0 0|| exit $?
				send_hlp_check  100 0x20004037 0x37 16 1 0 0 0|| exit $?
				rm -rf /mnt/jffs2/pcieloopbackflag
			else
				echo "ERROR::input para is not right!"
				exit 1
			fi
		        ...
		        check_param 5 || exit $?
		        send_hlp_check  100 0x20004037 0x37 16 $3 2 $4 $5|| exit $?
		elif [ "$2" = "result" ];then
			send_hlp_check  100 0x20004037 0x37 16 0 3 0 0|| exit $?
			rm -rf /mnt/jffs2/pcieloopbackflag
		elif [ "$2" = "maskwifi" ];then
			touch /mnt/jffs2/pcieloopbackflag
		else
		        echo "ERROR::Command is not right!"
		        exit 1
                fi
		...
		if [ "$2" = "-h" ];then
			...
			exit 0
                        ...
                        send_hlp 100 0x2000502c 0x2c 8 1 "$3" || exit $?
		elif [ "$2" = "result" ];then
                        send_hlp 100 0x2000502c 0x2c 8 2 0 || exit $?
		...
		if [ "$2" = "-h" ];then
			...
			check_param 3 || exit $?
			send_hlp_check 200 0x2003d001 1 4 "$3" || exit $?
		elif [ "$2" = "display" ];then
			send_hlp_check 200 0x2003d002 2 4 0 || exit $?
		else
			echo "ERROR::input para is not right!"
			exit 1
		fi 
