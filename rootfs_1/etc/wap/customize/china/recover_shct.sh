...
HW_Script_SetDatToFile()
{
	...
	var_access_limit=InternetGatewayDevice.Service.X_HW_AccessLimit
	...
	HW_Script_Cfgtool_Set $var_default_ctree $var_node_ssid "SSID" $var_ssid
	HW_Script_Cfgtool_Set $var_default_ctree $var_node_wpa_pwd "PreSharedKey" $var_wpa

	#上海电信xgon下挂设备最大连接数改为30
	var_xgpon_mode=`cat $var_boardinfo_dup |grep "obj.id = \"0x00000001\"" |cut -d ";" -f 2 | cut -d "\"" -f 2 `
	var_xgpon_terminal_number=30
	if [ $var_xgpon_mode = 5 ]
	then
		HW_Script_Cfgtool_Set $var_default_ctree $var_access_limit "TotalTerminalNumber" $var_xgpon_terminal_number
	fi
	...
	if [ 0 -eq $? ];then
		...
		echo "add $var_node_app_ftpuser FtpUserName useradmin" >> $var_batch_file
	...
	if [ 0 -eq $? ];then
		...
		echo "add $var_node_app_srvmng FtpUserName useradmin" >> $var_batch_file

		#add app srvmng account
		echo "add $var_node_app_srvmng FtpPassword $var_userPwd" >> $var_batch_file
	fi

	cfgtool find $var_default_ctree $var_node_app_smbuser
	if [ 0 -eq $? ];then
		# set app smb account
		echo "set $var_node_app_smbuser Password $var_userPwd" >> $var_batch_file
	fi
