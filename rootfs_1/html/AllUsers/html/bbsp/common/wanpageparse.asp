        ...
        for (var i = 0;i < GetWanList().length; i++)
        {
            ...
            {		
				if (ProductType == '2')
				{
					...
					else if ('JSCMCC' == CfgModeWord.toUpperCase() && Wan[i].VlanId == 4031 && Wan[i] && Wan[i].ServiceList == 'OTHER' && Wan[i].EncapMode == 'PPPoE' && Instance_IspWlan.IsWanHidden(domainTowanname(Wan[i].domain)) == true)
					{
					    document.write('<TD align="center">' + '<input type="checkbox" name="rml"'  + ' value="' + Wan[i].domain + '" onClick="selectRemoveCnt(this);" disabled="true">' + '</TD>');
					}
					else
...
function ParsePageControl(userPara)
{
	...
	if (ProductType == '2' && 'TELECOM' == CfgModeWord.toUpperCase())
	{
...
function ParsePageSpec()
{
	...
	if ((GetCfgMode().TELMEX != "1") && (CfgModeWord.toUpperCase() != "DTTNET2WIFI" && CfgModeWord.toUpperCase() != "TTNET2"))
	{
		if(1 == IsTedata)
		{
