...
function MakeWanNameForCtNewWan(Wan)
{
    var currentWanName = '';
    var linktype = '';
    if(Wan.WanAccessType == 'DSL') {
        ...
        linktype = "GE";
    } else if((Wan.WanAccessType == 'SFP') && (isSupportSFP == 1)) {
        linktype = "SFP";
    }
    ...
    if (!IsRadioWanSupported(Wan)) {
        if (Wan.ServiceList.toUpperCase().indexOf("INTERNET") >= 0) {
            currentWanName = "INTERNET_" + linktype;
        } else if (Wan.ServiceList.toUpperCase().indexOf("TR069") >= 0) {
            currentWanName = "Management_" + linktype;
        } else if (Wan.ServiceList.toUpperCase().indexOf("VOIP") >= 0) {
            currentWanName = "VOIP_" + linktype;
        } else {
            currentWanName = "IPTV_" + linktype;
        }
    } else {
...
function MakeWanNameForCT(Wan)
{
    if (Wan.MacId <= 9) {
        return MakeWanNameForCtNewWan(Wan);
    } else {
...
function MakeWanName_New(wan) {
    ...
    } else if (CfgModeWord.toUpperCase() == "DCTMNE2WIFI") {
        currentWanName = MakeWanNameForCT(wan);
...
function WanStatsLoader() {
    ...
    this.Validate = function() {
        timeWanStat = setInterval("wanStatsEvt()", 300);
    }
}

function wanStatsEvt() {
