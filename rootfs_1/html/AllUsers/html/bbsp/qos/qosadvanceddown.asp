...
var CmccFlag = ('CMCC' == CurrentBin.toUpperCase()) ? true : false ;
...
var QosBasicInfoListDown = GetQosBasicInfoListDown();
var AppInfoList = GetAppInfoList();
var ClassificationInfoList = GetClassificationInfoListDown();
var ClassificationTypeInfoList = GetClassificationTypeInfoListDown();
ClassificationTypeInfoList = QosClassificationInfoAdaptor(ClassificationTypeInfoList);

var PriorityQueueInfoListDown = GetPriorityQueueInfoListDown();
var SortAppIdList = new Array();
SortAppIdList = GetSortAppInfoList();
var SortClassIdList = new Array();
SortClassIdList = GetSortClassInfoListDown();
...
function setCtlDisplayDataClass(record)
{
    ...
    if (record.Domain == '')
    {
        ...
        if ((null != QosBasicInfoListDown[0]) && (1 == QosBasicInfoListDown[0].X_HW_EnableDSCPMark))
        {
        ...
        if ((null != QosBasicInfoListDown[0]) && (2 == QosBasicInfoListDown[0].X_HW_Enable8021p))
        {
...
{
    if (null == QosBasicInfoListDown[0])
    {
        if (E8CQoSMode == 1)
        {
            setText("Bandwidth", 0);
    ...
    else
    {
        if (E8CQoSMode == 1)
        {
            setText("Bandwidth", parseInt(QosBasicInfoListDown[0].X_HW_Bandwidth,10));
            setSelect("QoSPlan_select", QosBasicInfoListDown[0].X_HW_Plan);
            if (1 == QosBasicInfoListDown[0].X_HW_EnableDSCPMark)
            {
            ...
            setSelect("8021_PMarkType_select", QosBasicInfoListDown[0].X_HW_Enable8021p);
        }
    }
}
...
{
    var QueueIndex = '';
    if (PriorityQueueInfoListDown[index].Domain.length == 0)
    {
        QueueIndex = PriorityQueueInfoListDown[index].Priority;
    }
    else
    {
        QueueIndex = PriorityQueueInfoListDown[index].Domain.charAt(PriorityQueueInfoListDown[index].Domain.length - 1);
    }
    return QueueIndex;
}
...
function DisplayQueueEnable(plan)
{
    ...
    for (var i = 0; i < PriorityQueueInfoListDown.length; i++)
    {
        if (plan == "weight")
        {
            QueueId = 'WRRRecord' + i;
            ...
            setCheck(QueueId,PriorityQueueInfoListDown[i].Enable);
            setText(WeightId, PriorityQueueInfoListDown[i].Weight);
        }
        else
        {
            QueueId = 'SPRecord' + i;
...
{
    if (null != PriorityQueueInfoListDown)
    {
        for (var i = 0; i < PriorityQueueInfoListDown.length; i++)
        {
            var QueueId = "";
            var WeightId = "";
            if (QosBasicInfoListDown[0] == null)
            {
                QueueId = 'SPRecord' + i;
                setCheck(QueueId,PriorityQueueInfoListDown[i].Enable);
            }
            ...
            {
                DisplayQueueEnable(QosBasicInfoListDown[0].X_HW_Plan);
            }
        }
    }
}
...
{
    var InstanceId = Id + 1;
    if (PriorityQueueInfoListDown[Id].Domain != '')
    {
        return ;
    }
    ...
    async : false,
    cache : false,
    data : 'o.Enable=' + PriorityQueueInfoListDown[Id].Enable + '&o.Priority=' + PriorityQueueInfoListDown[Id].Priority 
           + '&o.Weight=' + PriorityQueueInfoListDown[Id].Weight + '&x.X_HW_Token=' + getValue('onttoken'),
    url :  'add.cgi?o=InternetGatewayDevice.X_HW_DownlinkQos.PriorityQueue'
      + '&RequestFile=html/bbsp/qos/qosadvanceddown.asp',
    error:function(XMLHttpRequest, textStatus, errorThrown) 
    {
        if (XMLHttpRequest.status == 404)
        {
            PriorityQueueInfoListDown[Id].Domain = 'InternetGatewayDevice.X_HW_DownlinkQos.PriorityQueue'+ InstanceId;
        }
    }
    });
}
...
function Insertoptions(startid,endid)
{
    ...
    {
        ArrWlan.push(parseInt(WlanInfoList[i].name.charAt(WlanInfoList[i].name.length -1),10) + 1);
    }
    ArrWlan.sort();
...
function OnQMApply()
{
    ...
    if (E8CQoSMode == 1)
    {
        ...
        if (Bandwidth!="")
        {
           if (false == CheckNumber(Bandwidth, 0, 2500000))
           {
             AlertEx(qos_language['bbsp_bandwidthdowninvaild']);
             return false;
           }
        }
        else
    ...
    for (i = 0; i < QueueNum; i++) 
    {
        ...
        url += "&y" + i + "=InternetGatewayDevice.X_HW_DownlinkQos.PriorityQueue." + (i+1);
    }
    ...
    Form.setAction('complex.cgi?' + '&x=InternetGatewayDevice.X_HW_DownlinkQos' + url + '&RequestFile=html/bbsp/qos/qosadvanceddown.asp');
...
function OnClassApply()
{
    ...
    Form.addParameter('x.8021pValue',getSelectVal('Class8021pList'));
    Form.addParameter('x.X_HW_Token', getValue('onttoken'));
    if (classIdx == -1)
    {
        Form.setAction('add.cgi?x=InternetGatewayDevice.X_HW_DownlinkQos.Classification' + '&RequestFile=html/bbsp/qos/qosadvanceddown.asp');
    }
    else
    {
        Form.setAction('set.cgi?x=' + ClassificationInfoList[classIdx].Domain + '&RequestFile=html/bbsp/qos/qosadvanceddown.asp');
    }
...
function OnClickDataClassModify()
{
    ...
    Form.addParameter('x.8021pValue',Class8021p);

    if (dataclassIdx == -1)
    {
        ClassUrl = 'x=InternetGatewayDevice.X_HW_DownlinkQos.Classification';
    }
    else
    {
    ...
    if (dataclassIdx == -1)
    {
        Form.setAction('addcfg.cgi?' + ClassUrl + ClassTypeUrl + '&RequestFile=html/bbsp/qos/qosadvanceddown.asp');
    }
    else
    {
        Form.setAction('complex.cgi?' + ClassUrl +  ClassTypeUrl + '&RequestFile=html/bbsp/qos/qosadvanceddown.asp');
    }
    Form.submit();
    DisableRepeatSubmit();
}
...
function OnClickDataClassDel() 
{
    ...
    if (0 == ClassNum)
    {
        ...
        Form.addParameter('x.X_HW_Token', getValue('onttoken'));
        Form.setAction('del.cgi?' + '&RequestFile=html/bbsp/qos/qosadvanceddown.asp');
    }
    else if (ClassNum > 0)
    {
        ...
        Form.addParameter('Del_z.8021pValue','');
        ClassUrl = "&Del_z="+ClassificationInfoList[idx].Domain;
        Form.addParameter('x.X_HW_Token', getValue('onttoken'));
        Form.setAction('complex.cgi?' +ClassTypeUrl+ ClassUrl+'&RequestFile=html/bbsp/qos/qosadvanceddown.asp');
    }
    setDisable('ClassificationModifySubmit_button',1);
    ...
    <div id="DivQueueManagement" style="display:none">
        ...
        <div id="DivQosBasic">
            <table cellpadding="0" cellspacing="0" width="100%" border="0" > 
                <tr id="BandwidthRow" style="display:none">
                  <td  class="table_title width_25p" BindText='bbsp_Bandwidthdown'></td> 
                ...
                <script>
                getElById("Bandwidth").title = qos_language['bbsp_Bandwidthdowntitle'];
                </script>
                <tr id="PlanRow">
        ...
        <div id="DivSP" style="display:none">
            <table width="100%" border="0" cellpadding="0" cellspacing="1" class="tabal_bg" id="priQueueInst"> 
                ...
                <script language="JavaScript" type="text/javascript">
                    if (PriorityQueueInfoListDown.length == 0)
                    {
                    ...
                    {
                        for (i = 0; i < PriorityQueueInfoListDown.length; i++)
                        {
                            document.write('<TR id="SPrecord_' + i + '" class="tabal_01">');
                            document.write('<TD align="center" width="5%">' + '<input type="checkbox" name="SPrml" id="SPRecord' + i + '"+ value="'+ PriorityQueueInfoListDown[i].Enable+'">' + '</TD>');
                            document.write('<TD align="center" width="40%">' + 'Q' + PriorityQueueInfoListDown[i].Priority + '</TD>');
                            document.write('<TD align="center" width="60%">' + PriorityQueueInfoListDown[i].Priority + '</TD>');
                            document.write('</TR>');
                        }
                    }
                </script>
                ...
                <script language="JavaScript" type="text/javascript">
                    ...
                    else
                    {
                        for (i = 0; i < PriorityQueueInfoListDown.length; i++)
                        {
                            ...
                            if (PriorityQueueInfoListDown[i].Domain.length == 0)
                            {
                                QueueIndex = PriorityQueueInfoListDown[i].Priority;
                            }
                            else
                            {
                                QueueIndex = PriorityQueueInfoListDown[i].Domain.charAt(PriorityQueueInfoListDown[i].Domain.length - 1);
                            }
                            
                            document.write('<TR id="WRRrecord_' + i + '" class="tabal_01">');
                            document.write('<TD align="center" width="5%">' + '<input type="checkbox" name="WRRrml" id="WRRRecord' + i + '"+ value="'+ PriorityQueueInfoListDown[i].Enable+'">' + '</TD>');
                            document.write('<TD align="center" width="40%">' + 'Q' + QueueIndex + '</TD>');
                            document.write('<TD align="center" width="60%">' + '<input type="text" id="Q' + QueueIndex +'Weight_text" size="3" maxlength="3" ' + 'value="' + PriorityQueueInfoListDown[i].Weight + '"/>'+'</TD>');
                            document.write('</TR>');
                        }
                    }
                </script>
        ...
        <script language="JavaScript" type="text/javascript">
            ...
            {
                if (QosBasicInfoListDown[0].X_HW_Plan == "weight")
                {
                    setDisplay("DivWRR",1);
                }
                else
                        ...
                        else
                        {
                            ...
                            for (var j = 0; j < ClassNum; j++)
                            {
                                ...
                                if (DataClasslist[j].Type == "LANInterface")
                                {
                                    if (0 == STBPort)
                                    {
                                        if (DataClasslist[j].MinShow == "千兆口2" || DataClasslist[j].MinShow == "百兆口2")
                                        {
                                            DataClasslist[j].MinShow = "iTV";
                                        }
                                        if (DataClasslist[j].MaxShow == "千兆口2" || DataClasslist[j].MaxShow == "百兆口2")
                                        {
                                            DataClasslist[j].MaxShow = "iTV";
                                        }
                                    }
                                    else
                                    {
                                        if (DataClasslist[j].MinShow == "千兆口3" || DataClasslist[j].MinShow == "百兆口3")
                                        {
                                            DataClasslist[j].MinShow = "STB";
                                        }
                                        if (DataClasslist[j].MaxShow == "千兆口3" || DataClasslist[j].MaxShow == "百兆口3")
                                        {
                ...
                <table cellpadding="0" cellspacing="0" border="0" id="DataClass1_table" width="100%"> 
                    <tr>
                        ...
                        <td  class="table_right"><select name='Type1_select'  id="Type1_select" size='1' onChange="setQosClassType(this.id);"> 
                            ...
                            <option value="WANInterface"><script>document.write(qos_language['bbsp_WANInterface1']);</script></option>
                            <script language="JavaScript" type="text/javascript">
                            if (true == CmccFlag)
                            {
